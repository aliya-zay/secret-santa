...
class _RoomPageState extends State<RoomPage> {
  ...
  Future<void> _performDraw() async {
    if (_roomData == null || _roomData!.length < 4) {
      setState(() {
        _errorMessage = 'Необходимо как минимум 4 участника для розыгрыша.';
      });
      return;
    }
    ...
    final participants = _roomData!
        ...
        .map((p) => p['email'])
        .toList();
    final admin = _roomData!.firstWhere((p) => p['is_admin'], orElse: () => {});
  ...
  Widget _buildAssignments() {
    ...
    final isAdmin = _roomData?.firstWhere((p) => p['email'] == myEmail, orElse: () => {})['is_admin'] ?? false;
